...
if (isset($_POST['save'])) {
	...
	if (mysqli_num_rows($result) == 0) {
		...
		if(mysqli_query($con, $query)) {
        	header('location:events?message=addsuccess');
   			exit;	
        } else {
          header('location:events?message=addfailed');
   		 exit;
...
if (isset($_POST['delete'])) {
	...
	if ($result) {
	   header('location:events?message=delete');
	   exit;
	  }
	  else{
...
if (isset($_POST['edit'])) {
	...
	$query = "UPDATE events SET EVENT = '$event', SCHEDULE = '$date', LOCATION = '$location', DESCRIPTION ='$desc' WHERE ID = '$id'";
		...
		if(mysqli_query($con, $query)) {
        	header('location:events?message=updatesuccess');
   			exit;	
        } else {
          header('location:events?message=addfailed');
   		 exit;
