...
<ul class="breadcrumb">
  ...
  <li><a href="alumni_list">Registered Alumni</a></li>
...
<div class="modal fade" id="editmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	 ...
	 <div class="modal-footer">
	 			<a href="emp_history?email=<?php echo $row['EMAIL'];?>" class="edit btn btn-primary" title="view employment history">Employment History</a>
	 			...
	 			<button type="button" class="edit btn btn-primary" data-toggle="modal" data-target="#editmodal" title="Edit" data-mdb-toggle="popover" data-mdb-trigger="hover">Edit Information</button>
		      	<a href="alumni_list"><button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button> </a>
		      </div>
</div>	
</div>	
</div>
...
if (isset($_POST['edit'])) {
	...
	$query = "UPDATE alumni SET ID_NUMBER = '$id_num', LASTNAME = '$lastname', FIRSTNAME = '$firstname', MIDDLENAME ='$middlename', COURSE = '$program', MAJOR = '$major', BATCH= '$batch', GENDER = '$gender', EMAIL = '$email', BIRTHDAY = '$birthday', CONTACT = '$contact', ADDRESS = '$address' WHERE ID = '$id'";
		...
		if(mysqli_query($con, $query)) {
        	header('location:alumni_list?message=updatesuccess');
   			exit;	
        } else {
          header('location:alumni_list?message=addfailed');
   		 exit;
...
if (isset($_POST['change'])) {
    ...
    if(mysqli_query($con, $query)) {
          ...
          header('location:viewprofle?message=addfailed');
   		 exit;
