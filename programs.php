...
if (isset($_POST['save'])) {
	...
	if (mysqli_num_rows($result) == 0) {
		...
		if(mysqli_query($con, $query)) {
        	header('location:programs?message=addsuccess');
   			exit;	
        } else {
          header('location:programs?message=addfailed');
   		 exit;
...
if (isset($_POST['delete'])) {
	...
	if ($result) {
	   header('location:programs?message=delete');
	   exit;
	  }
	  else{
...
if (isset($_POST['edit'])) {
	...
	$query = "UPDATE programs SET TITLE = '$title', ABBREVIATION = '$abbr', DESCRIPTION ='$desc' WHERE ID = '$id'";
		...
		if(mysqli_query($con, $query)) {
        	header('location:programs?message=updatesuccess');
   			exit;	
        } else {
          header('location:programs?message=addfailed');
   		 exit;
